...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.RyderwinstonOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("Ryderwinston | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if callSafely(isfolder, RyderwinstonFolder) then
				if callSafely(isfile, RyderwinstonFolder..'/settings'..ConfigurationExtension) then
					file = callSafely(readfile, RyderwinstonFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"RyderwinstonOpen":{"Value":"K","Type":"bind","Name":"Ryderwinston Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Ryderwinston Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Ryderwinston had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled then
	...
	if cachedSettings and (#cachedSettings == 0 or (cachedSettings.System and cachedSettings.System.usageAnalytics and cachedSettings.System.usageAnalytics.Value)) then
		sendReport("execution", "Ryderwinston")
...
local Ryderwinston = useStudio and script.Parent:FindFirstChild('Ryderwinston') or game:GetObjects("rbxassetid://10804731440")[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local RyderwinstonDestroyed = false -- True when RyderwinstonLibrary:Destroy() is called

repeat
	if Ryderwinston:FindFirstChild('Build') and Ryderwinston.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Ryderwinston | Build Mismatch')
		print('Ryderwinston may encounter issues as you are running an incompatible interface version ('.. ((Ryderwinston:FindFirstChild('Build') and Ryderwinston.Build.Value) or 'No Build') ..').\n\nThis version of Ryderwinston is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Ryderwinston = Ryderwinston, useStudio and script.Parent:FindFirstChild('Ryderwinston') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Ryderwinston.Enabled = false

if gethui then
	Ryderwinston.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Ryderwinston)
	Ryderwinston.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Ryderwinston.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Ryderwinston.Name and Interface ~= Ryderwinston then
			Interface.Enabled = false
			Interface.Name = "Ryderwinston-Old"
		end
	end
elseif not useStudio then
...
if Ryderwinston.AbsoluteSize.X < minSize.X and Ryderwinston.AbsoluteSize.Y < minSize.Y then
...
local Main = Ryderwinston.Main
local MPrompt = Ryderwinston:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Ryderwinston:FindFirstChild('Drag')
...
local Notifications = Ryderwinston.Notifications
local keybindConnections = {} -- For storing keybind connections to disconnect when Ryderwinston is destroyed

local SelectedTheme = RyderwinstonLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = RyderwinstonLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Ryderwinston.Main.BackgroundColor3 = SelectedTheme.Background
	Ryderwinston.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Ryderwinston.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Ryderwinston.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Ryderwinston.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Ryderwinston.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Ryderwinston.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Ryderwinston.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Ryderwinston.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Ryderwinston:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Ryderwinston | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Ryderwinston | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Ryderwinston had an issue decoding the configuration file, please try delete the file and reopen Ryderwinston.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(RyderwinstonLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Ryderwinston | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RyderwinstonLibrary:Notify({Title = "Ryderwinston Flags", Content = "Ryderwinston was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(RyderwinstonLibrary.Flags) do
...
function RyderwinstonLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Ryderwinston | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RyderwinstonLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			RyderwinstonLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {getSetting("General", "RyderwinstonOpen")}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, RyderwinstonFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Ryderwinston Settings', 0, true)

	if TabList['Ryderwinston Settings'] then
		TabList['Ryderwinston Settings'].LayoutOrder = 1000
	end

	if Elements['Ryderwinston Settings'] then
		Elements['Ryderwinston Settings'].LayoutOrder = 1000
	end
...
function RyderwinstonLibrary:CreateWindow(Settings)
	if Ryderwinston:FindFirstChild('Loading') then
		if getgenv and not getgenv().RyderwinstonCached then
			Ryderwinston.Enabled = true
			Ryderwinston.Loading.Visible = true

			task.wait(1.4)
			Ryderwinston.Loading.Visible = false
		end
	end

	if getgenv then getgenv().RyderwinstonCached = true end
	...
	if not Settings.DisableRyderwinstonPrompts then
		task.spawn(function()
			while true do
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio then
		ensureFolder(RyderwinstonFolder.."/Discord Invites")

		if callSafely(isfile, RyderwinstonFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, RyderwinstonFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Ryderwinston RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(RyderwinstonFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Ryderwinston | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
				end
			end
		end
		...
		if callSafely(isfile, RyderwinstonFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, RyderwinstonFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, RyderwinstonFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						RyderwinstonLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called RyderwinstonLibrary:Destroy()
				if RyderwinstonDestroyed then
					return
				end
				if not Success then
					...
					print("Ryderwinston | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= RyderwinstonLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					RyderwinstonLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Ryderwinston.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Ryderwinston | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					RyderwinstonLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Ryderwinston | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					RyderwinstonLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Ryderwinston | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					RyderwinstonLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= RyderwinstonLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Ryderwinston | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						RyderwinstonLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Ryderwinston | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Ryderwinston specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					RyderwinstonLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			RyderwinstonLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			RyderwinstonLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Ryderwinston had an issue creating settings.') end
...
function RyderwinstonLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function RyderwinstonLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function RyderwinstonLibrary:Destroy()
	RyderwinstonDestroyed = true
	hideHotkeyConnection:Disconnect()
	for _, connection in keybindConnections do
		connection:Disconnect()
	end
	Ryderwinston:Destroy()
end
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Ryderwinston Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "RyderwinstonOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function RyderwinstonLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RyderwinstonLibrary:Notify({Title = "Ryderwinston Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RyderwinstonLibrary:Notify({Title = "Ryderwinston Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Ryderwinston Configurations Error | '..tostring(result))
			RyderwinstonLibrary:Notify({Title = "Ryderwinston Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = RyderwinstonLibrary:CreateWindow({
	--	Name = "Ryderwinston Example Window",
	--	LoadingTitle = "Ryderwinston Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Ryderwinston may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Ryderwinston to get the key from
	...
	----RyderwinstonLibrary:Notify({Title = "Ryderwinston Interface", Content = "Welcome to Ryderwinston. These - are the brand new notification design for Ryderwinston, with custom sizing and Ryderwinston calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(RyderwinstonLibrary.Theme) do
...
task.delay(4, function()
	RyderwinstonLibrary.LoadConfiguration()
